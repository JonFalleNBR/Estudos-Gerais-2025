function funChallenge(input){
    ...
    for (let i = 0; i < input.length; i++){ // here the algorithm turns into O(n) because the time of execution of this code depends on the size of the input, and we have to execute it n times, where n is the size of the input
        anotherFunction(); // O(1)  because it is a function that is executed only once, and it does not depend on the size of the input
        let stranger = true 
        a++ 
...
Answering questions 
    1 - what this codes does ?  This code starts with a variable assignaded as 10, then , it reassings as 53 - this still O(1) because it doesn`t matter how the size of the input, the value of a still 53 
                But, when we have to start a loop to interate the input.length, the time complexity of the code increases to O(N), due the fact that all the line codes inside this loops depends of the size of the input and the number o f times that we have to exexute it 
